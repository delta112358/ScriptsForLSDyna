...
def evaluateAcceleration(t,maximumDisplacement,endTime):
    return round(8*maximumDisplacement*(math.sin(2*t*math.pi/endTime))**2/endTime**2,10)

def evaluateDisplacement(t,maximumDisplacement,endTime):
    return round(maximumDisplacement*(2*(t/endTime)**2+(math.cos((4*t*math.pi)/endTime)-1)/(4*(math.pi)**2)),10)

def generateAccelerationValueList(maximumDisplacement,endTime,TimeList):
    accelerationList=[]
    for t in Timelist:
        acceleration = evaluateAcceleration(t,maximumDisplacement,endTime)
        if t<= endTime/2:
...
def generateDisplacementValueList(maximumDisplacement,endTime,TimeList):
    return[evaluateDisplacement(t,maximumDisplacement,endTime) if t<=endTime/2 else -evaluateDisplacement(t,maximumDisplacement,endTime) for t in TimeList]
